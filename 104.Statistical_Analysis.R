...
# I'M CONSIDERING THE SUM OF THE CONFLICTS FOR EACH REGION AND YEAR
...
for (continent in world){
  ...
  a <- fixest::feglm(data = data_continent, all_confl ~ value | region + year, family = quasipoisson)
  glm[[continent]] <- a 
}
North_America <- c("United States", "Canada", "Mexico")
N_A <- gem[gem$country %in% North_America, ]
a <- fixest::feglm(data=N_A, all_confl~value|region + year, family=quasipoisson)
glm[["North_America"]] <- a
...
a <- fixest::feglm(data=S_A, all_confl~value|region + year, family=quasipoisson)
glm[["South America"]] <- a 
...
for (continent in world){
  ...
  data_continent <- events_sum %>%
    filter(get_continent(country) == continent)
  a <- fixest::feglm(data = data_continent, all_confl ~ mvalue1 | region + year, family = quasipoisson)
  glm[[continent]] <- a 
}
North_America <- c("United States", "Canada", "Mexico")
N_A <- gem[gem$country %in% North_America, ]
a <- fixest::feglm(data=N_A, all_confl~mvalue1|region + year, family=quasipoisson)
glm[["North_America"]] <- a
South_America <- c("Belize", "Costa Rica", "El Salvador", "Guatemala", "Honduras", "Nicaragua", "Panama", "Argentina", "Bolivia", "Brasile", "Cile", "Colombia", "Ecuador", "Guyana", "Paraguay", "Perù", "Suriname", "Uruguay", "Venezuela", "Aruba", "Bahamas", "Barbados", "Cuba", "Dominica", "Giamaica", "Haiti", "Trinidad and Tobago", "Sint Maarten", "Saint Vincent and the Grenadines", "Saint Lucia", "Saint Kitts and Nevis", "Puerto Rico", "Dominican Republic", "Grenada", "Martinique", "Saint Martin", "Virgin Islands", "Turks and Caicos Islands", "Cayman Islands", "British Virgin Islands", "Guadeloupe", "Antigua and Barbuda", "Bonaire", "Curacao", "Saint Barthelemy", "Saba", "Saint Eustatius", "Saint Pierre and Miquelon", "Falkland Islands", "French Guiana", "South Georgia and the South Sandwich Islands", "British West Indies")
S_A <- gem[gem$country %in% South_America, ]
a <- fixest::feglm(data=S_A, all_confl~mvalue1|region + year, family=quasipoisson)
glm[["South America"]] <- a 
table <- etable(glm, tex = TRUE)
...
for (continent in world){
  ...
  data_continent <- events_sum %>%
    filter(get_continent(country) == continent)
  a <- fixest::feglm(data = data_continent, all_confl ~ mvalue5 | region + year, family = quasipoisson)
  glm[[continent]] <- a 
}
North_America <- c("United States", "Canada", "Mexico")
N_A <- events_sum[events_sum$country %in% North_America, ]
a <- fixest::feglm(data=N_A, all_confl~mvalue5|region + year, family=quasipoisson)
glm[["North_America"]] <- a
South_America <- c("Belize", "Costa Rica", "El Salvador", "Guatemala", "Honduras", "Nicaragua", "Panama", "Argentina", "Bolivia", "Brasile", "Cile", "Colombia", "Ecuador", "Guyana", "Paraguay", "Perù", "Suriname", "Uruguay", "Venezuela", "Aruba", "Bahamas", "Barbados", "Cuba", "Dominica", "Giamaica", "Haiti", "Trinidad and Tobago", "Sint Maarten", "Saint Vincent and the Grenadines", "Saint Lucia", "Saint Kitts and Nevis", "Puerto Rico", "Dominican Republic", "Grenada", "Martinique", "Saint Martin", "Virgin Islands", "Turks and Caicos Islands", "Cayman Islands", "British Virgin Islands", "Guadeloupe", "Antigua and Barbuda", "Bonaire", "Curacao", "Saint Barthelemy", "Saba", "Saint Eustatius", "Saint Pierre and Miquelon", "Falkland Islands", "French Guiana", "South Georgia and the South Sandwich Islands", "British West Indies")
S_A <- events_sum[events_sum$country %in% South_America, ]
a <- fixest::feglm(data=S_A, all_confl~mvalue5|region + year, family=quasipoisson)
glm[["South America"]] <- a 
table <- etable(glm, tex = TRUE)
...
for (continent in world){
  ...
  data_continent <- events_sum %>%
    filter(get_continent(country) == continent)
  a <- fixest::feglm(data = data_continent, all_confl ~ vvalue1 | region + year, family = quasipoisson)
  glm[[continent]] <- a 
}
North_America <- c("United States", "Canada", "Mexico")
N_A <- events_sum[events_sum$country %in% North_America, ]
a <- fixest::feglm(data=N_A, all_confl~vvalue1|region + year, family=quasipoisson)
glm[["North_America"]] <- a
South_America <- c("Belize", "Costa Rica", "El Salvador", "Guatemala", "Honduras", "Nicaragua", "Panama", "Argentina", "Bolivia", "Brasile", "Cile", "Colombia", "Ecuador", "Guyana", "Paraguay", "Perù", "Suriname", "Uruguay", "Venezuela", "Aruba", "Bahamas", "Barbados", "Cuba", "Dominica", "Giamaica", "Haiti", "Trinidad and Tobago", "Sint Maarten", "Saint Vincent and the Grenadines", "Saint Lucia", "Saint Kitts and Nevis", "Puerto Rico", "Dominican Republic", "Grenada", "Martinique", "Saint Martin", "Virgin Islands", "Turks and Caicos Islands", "Cayman Islands", "British Virgin Islands", "Guadeloupe", "Antigua and Barbuda", "Bonaire", "Curacao", "Saint Barthelemy", "Saba", "Saint Eustatius", "Saint Pierre and Miquelon", "Falkland Islands", "French Guiana", "South Georgia and the South Sandwich Islands", "British West Indies")
S_A <- events_sum[events_sum$country %in% South_America, ]
a <- fixest::feglm(data=S_A, all_confl~vvalue1|region + year, family=quasipoisson)
glm[["South America"]] <- a 
table <- etable(glm, tex = TRUE)
...
for (continent in world){
  ...
  data_continent <- events_sum %>%
    filter(get_continent(country) == continent)
  a <- fixest::feglm(data = data_continent, all_confl ~ sdvalue5 | region + year, family = quasipoisson)
  glm[[continent]] <- a 
}
North_America <- c("United States", "Canada", "Mexico")
N_A <- events_sum[events_sum$country %in% North_America, ]
a <- fixest::feglm(data=N_A, all_confl~sdvalue5|region + year, family=quasipoisson)
glm[["North_America"]] <- a
South_America <- c("Belize", "Costa Rica", "El Salvador", "Guatemala", "Honduras", "Nicaragua", "Panama", "Argentina", "Bolivia", "Brasile", "Cile", "Colombia", "Ecuador", "Guyana", "Paraguay", "Perù", "Suriname", "Uruguay", "Venezuela", "Aruba", "Bahamas", "Barbados", "Cuba", "Dominica", "Giamaica", "Haiti", "Trinidad and Tobago", "Sint Maarten", "Saint Vincent and the Grenadines", "Saint Lucia", "Saint Kitts and Nevis", "Puerto Rico", "Dominican Republic", "Grenada", "Martinique", "Saint Martin", "Virgin Islands", "Turks and Caicos Islands", "Cayman Islands", "British Virgin Islands", "Guadeloupe", "Antigua and Barbuda", "Bonaire", "Curacao", "Saint Barthelemy", "Saba", "Saint Eustatius", "Saint Pierre and Miquelon", "Falkland Islands", "French Guiana", "South Georgia and the South Sandwich Islands", "British West Indies")
S_A <- events_sum[events_sum$country %in% South_America, ]
a <- fixest::feglm(data=S_A, all_confl~sdvalue5|region + year, family=quasipoisson)
glm[["South America"]] <- a 
table <- etable(glm, tex = TRUE)
